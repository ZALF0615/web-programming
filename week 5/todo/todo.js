...
switch(item, direction) {
    const index = this.items.indexOf(item);
    if (index !== -1) {
      const targetIndex = index + (direction ? 1 : -1); // 이동할 인덱스 계산
      if (targetIndex >= 0 && targetIndex < this.items.length) {
        // 배열 범위 내에서만 순서를 바꿀 수 있음
        this.items.splice(index, 1); // 현재 위치에서 항목 삭제
        this.items.splice(targetIndex, 0, item); // 이동된 위치에 항목 추가
        this.save(); // 로컬 스토리지에 항목 리스트 저장
      }
    }
  },
